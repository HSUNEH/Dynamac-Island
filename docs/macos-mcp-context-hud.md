...
The module also includes `compareDisabledMacContextHudRegression()` for the opt-out path. `scripts/test-mac-context-disabled-hud-regression-comparison.js` builds a main-like payload and an experimental payload with Mac Context explicitly disabled through both `enableMacContext: false` and `DYNAMAC_DISABLE_MAC_CONTEXT_HUD=1`, then verifies the existing status ordering, compact surface, and ranked Dynamic Island activities are unchanged. Run it independently with `npm run test:mac-context-disabled-hud-regression-comparison`.

The same comparison module also carries reliability checks for unavailable and stale Mac Context snapshots. `compareStaleMacContextHudReliability()` treats a status-source payload older than the configured local freshness threshold as stale, and the test verifies that stale active app/window data is downgraded to a warning HUD activity with user-visible stale/degraded text instead of crashing or silently showing it as fresh. Run it with `npm run test:mac-context-reliability-comparison`.

`src/mac-context-permission-burden-comparison.js` is the runnable permission-burden/degradation-state comparison module. It compares the same required macOS permission keys for main and the experimental branch: `accessibility` and `screenRecording`. Main is recorded as `notRequired` for both because it has no Mac Context HUD contract, and also carries an explicit non-degraded baseline so the test can compare degradation reporting shape without pretending main has the feature. The experimental branch must still report normalized statuses (`granted`, `denied`, or `unknown`) for both permissions, keep `degradationState` consistent with `result.degradation.state`, include user-visible degradation reasons when degraded, and mark Accessibility as required only for active-window/UI-tree enrichment while Screen Recording is reported without being required by this MVP. Run it with `npm run test:mac-context-permission-burden-comparison`.
...
npm run test:mac-context-disabled-hud-regression-comparison
