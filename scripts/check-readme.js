...
const requiredSnippets = [
  ...
  ["Mac activity runtime model", "the native launch path starts a local YouTube media bridge (`127.0.0.1:17654`) and the optional `extensions/youtube-media-bridge` content script sends the actual page player title/channel/duration/currentTime/playbackState"],
  ["YouTube real-player priority", "These real player sources beat stale/background Spotify"],
  ["YouTube extension launch", "launch with `npm run start:arc-media` or `npm run start:chrome-media`, allow YouTube loopback access to `127.0.0.1` when Arc asks"],
  ["Arc shared Space mode", "launches ST's normal Arc profile/account and relies on a dedicated Arc Space for Snuffles tabs"],
