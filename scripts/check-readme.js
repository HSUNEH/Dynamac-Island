...
const requiredSnippets = [
  ...
  ["YouTube arbitration and permissions", "YouTube browser probing prefers confirmed `playing` browser media, then native Spotify/Music, then title-only browser fallbacks; Chromium-family browsers may require the browser's `Allow JavaScript from Apple Events` setting plus macOS Automation approval. Run `npm run diagnose:youtube` to see which browser probe is blocked, and `npm run enable:browser-apple-events` to patch Chromium profile preferences when JavaScript-from-Apple-Events is disabled"],
