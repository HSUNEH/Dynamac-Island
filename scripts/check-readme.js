...
const requiredSnippets = [
  ...
  ["Mac activity runtime model", "`Now Playing` from macOS MediaRemote/`nowplaying-cli` current playback first, with a frontmost YouTube browser-window title fallback allowed to beat stale/background Spotify when Arc/Safari/Chrome/Firefox is visibly in front, then Spotify/Music best-effort AppleScript plus YouTube tab scanning across Safari, Chrome/Chromium-family browsers, and a Firefox-family title fallback with page metadata/video timing/thumbnail detection where the browser exposes tab JavaScript; `Clipboard` from local text clipboard, and `Battery` from `pmset -g batt`"],
  ["YouTube arbitration and permissions", "Run `npm run diagnose:youtube` to see which browser probe is blocked, and `npm run enable:browser-apple-events` to patch Chromium profile preferences when JavaScript-from-Apple-Events is disabled; if Arc exists but has no profile yet, the helper seeds a minimal `snuffles` profile"],
