...
assert.match(source, /var activeApp: String\?/, "native Mac Context status items should decode the active app for HUD display");
assert.match(source, /var activeWindow: String\?/, "native Mac Context status items should decode the active window for HUD display");
assert.match(source, /var permissionStatus: MacPermissionStatus\?/, "native Mac Context status items should decode permission status for degradation display");
assert.match(source, /drawMacContextActivity/, "native overlay should render a dedicated Mac Context HUD surface instead of only generic text");
assert.match(source, /macContextRenderedNotchOutput/, "native smoke contract should expose rendered Mac Context app, window, permission, and degradation text");
...
assert.match(source, /isRunning: status\.state == "running" && timer\.state == "running" && remainingSeconds > 0/, "compact Timer view model should mark only non-elapsed running timers as running");
...
assert.match(source, /focusBrowserMediaTab/, "browser media clicks should focus the already-open tab instead of opening a new one");
assert.match(source, /NSWorkspace\.shared\.open\(url\)/, "browser media clicks should fall back to opening the page URL only when the tab is not already open");
assert.match(source, /select t\n\s*activate/, "Arc tab focus should select the existing tab object");
assert.match(source, /set active tab index of w to i/, "Chromium tab focus should activate the matching tab by index");
assert.match(source, /set current tab of w to t/, "Safari tab focus should activate the matching tab");
...
assert.match(source, /performMediaSeek/, "native overlay should wire progress seeking to Spotify, Music, and browser media");
assert.match(source, /isBrowserMediaSource/, "native overlay should route browser media sources to MediaRemote control");
assert.match(source, /runNowPlayingCommand/, "browser media control should go through MediaRemote");
assert.match(source, /"nowplaying-cli"/, "MediaRemote control should shell out to nowplaying-cli");
assert.match(source, /"togglePlayPause"/, "browser media play/pause should use MediaRemote togglePlayPause");
assert.match(source, /"Google Chrome Canary"/, "browser tab focus should include Chrome Canary");
assert.match(source, /"Vivaldi"/, "browser tab focus should include Vivaldi");
assert.match(source, /"Opera GX"/, "browser tab focus should include Opera GX");
assert.match(source, /"Dia"/, "browser tab focus should include Dia");
assert.doesNotMatch(source, /execute t javascript/, "browser control must not use Arc-hanging execute-javascript");
