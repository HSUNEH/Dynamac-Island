...
- Mac activity snapshot model: `Now Playing` from Spotify/Music best-effort AppleScript plus YouTube tab scanning across Safari, Chrome/Chromium-family browsers, and a Firefox-family title fallback with page metadata/video timing/thumbnail detection where the browser exposes tab JavaScript; `Clipboard` from local text clipboard, and `Battery` from `pmset -g batt`. YouTube browser probing prefers confirmed `playing` browser media, then native Spotify/Music, then title-only browser fallbacks; Chromium-family browsers may require the browser's `Allow JavaScript from Apple Events` setting plus macOS Automation approval. Run `npm run diagnose:youtube` to see which browser probe is blocked, and `npm run enable:browser-apple-events` to patch Chromium profile preferences when JavaScript-from-Apple-Events is disabled; if Arc exists but has no profile yet, the helper seeds a minimal `snuffles` profile. Hermes runtime status remains an optional/dev provider, not the default product surface.
