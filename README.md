...
- Mac activity snapshot model: `Now Playing` first checks live YouTube player surfaces for Arc/Chrome playback: the native launch path starts a local YouTube media bridge (`127.0.0.1:17654`) and the optional `extensions/youtube-media-bridge` content script sends the actual page player title/channel/duration/currentTime/playbackState; `npm run start:arc-media` launches ST's normal Arc profile/account and relies on a dedicated Arc Space for Snuffles tabs, so the browser state intentionally stays in iamsuntae@gmail.com's Arc environment; frontmost Chrome/Safari can also read the same player values through browser JavaScript, and CDP (`127.0.0.1:9222-9225`, configurable with `DYNAMAC_CDP_PORTS`) is used when a debug-enabled Chromium browser exposes tabs. These real player sources beat stale/background Spotify. macOS MediaRemote/`nowplaying-cli`, Spotify/Music AppleScript, and title-only browser probes remain compatibility paths; title-only YouTube does not beat an actively playing native player. To force browser-extension mode for Arc-Snuffles or Chrome, launch with `npm run start:arc-media` or `npm run start:chrome-media`, allow YouTube loopback access to `127.0.0.1` when Arc asks, reload the YouTube tab, then run `npm run diagnose:youtube`. `Clipboard` comes from the local text clipboard, and `Battery` from `pmset -g batt`. Hermes runtime status remains an optional/dev provider, not the default product surface.
