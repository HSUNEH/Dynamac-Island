...
- Now Playing native UI: notch mode shows only album art/YouTube thumbnail/music-note fallback on the left wing plus a simple animated playing-bars indicator on the right wing; expanded mode shows the cover on the left, title, artist/source, smoothly advancing play time/progress, draggable/clickable seek, and previous/play-pause/next vector controls. Notch-to-expanded transitions animate only the lightweight island shell, then fade media content in after resize so artwork-heavy Now Playing surfaces do not stutter.
