...
- Activity Router core: `src/activity-router.js` normalizes status items into deterministic local activities with stable `activityId`, `activityType`, `priority`, timestamps, transient expiry, status payload, compact/expanded surface summaries, source, metadata, reveal-ready path, and persistence flags. Compact selection uses the priority order volume/brightness HUD > clipboard > shelf/drop > timer > Mac Context > now playing > battery/future passive, then updated recency, creation time, and ID for deterministic ties. This is testable core logic only: native global volume/brightness event capture, native drag capture, Finder reveal actions, and clipboard history persistence are not implied by the router.
- Experimental Mac Context HUD: `scripts/write-mac-activity-status.js` now emits a read-only `Mac Context` status item with active app, front window title when Accessibility permits it, summarized UI tree availability, Accessibility/Screen Recording permission diagnostics, and a user-visible `degradationState`. This follows the practical local macOS-MCP-style pattern of permission-first diagnostics without vendoring an MCP server, requiring remote services, bypassing macOS consent, or enabling screenshots/private hooks. See `docs/macos-mcp-context-hud.md` for the branch contract and comparison against main.
...
### DynaKeys Volume/Brightness HUD Core Model

`src/volume-hud-status.js` and `src/brightness-hud-status.js` implement the first DynaKeys volume/brightness HUD slice as deterministic pure logic. They accept observed local output-volume or display-brightness input changes (`level`, optional mute/display/device metadata, `source`, `observedAt`) and return one transient `Volume` or `Brightness` activity with stable `activityId`, `activityType`, `priority`, `createdAt`, `updatedAt`, `expiresAt`, `isTransient`, serializable `status`, compact and expanded surfaces, local `source`/`metadata`, empty `revealReadyPath`, and `persisted: false`.

`src/hud-event-store.js` is the local persistence companion for those HUD inputs. It appends normalized volume/brightness input events to a deterministic JSON store with `version`, `updatedAt`, stable `hud-<type>-<observedAt>-<sequence>` IDs, rounded levels, local device/display/source metadata, and a bounded `maxEvents` history. The writer uses local filesystem reads/writes only, creates parent directories, and writes atomically through a temp file + rename; it has no network, credential, cloud sync, paid API, clipboard text, or external state dependency. `src/mac-activity-status.js` can now capture emitted HUD inputs into that store with `hudEventStorePath`, replay recent unexpired HUD activity into a fresh status payload after restart, and continue burst direction/previous-level semantics from replayed state.

The implemented DynaKeys HUD behavior is intentionally compact and transient: volume changes render a `speaker.wave.2` or muted-speaker compact surface with a percentage label/progress, brightness changes render a `sun.max` compact surface with a percentage label/progress, and expanded surfaces describe the current level plus direction. The models keep repeated changes inside a short burst as one activity, derive `initial`/`up`/`down`/`steady` plus volume mute/unmute transitions from the previous observed value, and start a fresh activity after expiry so old HUD state does not leak into later bursts. They are covered by `npm run test:volume-hud-status`, `npm run test:brightness-hud-status`, `npm run test:hud-event-store`, and the Mac activity integration/restart replay checks in `npm run test:mac-activity-status`, all included in `npm run check`.

`npm run native:start` defaults the local HUD replay store to `.build/hud-events.json` (override with `DYNAMAC_HUD_EVENT_STORE`) so the app-mode writer can reuse the same replay path without changing the menu-bar `.app` architecture. The native-start writer now polls macOS `get volume settings` and emits a DynaKeys Volume HUD only when the output volume/mute state changes after its quiet baseline; set `DYNAMAC_HUD_EMIT_INITIAL=1` for diagnostics if you want the current level to appear immediately on launch, so changing the Mac volume can temporarily outrank Timer/Now Playing in the compact Activity Router. Brightness uses the same observer pipeline when a readable source is available: `DYNAMAC_BRIGHTNESS_LEVEL`, `DYNAMAC_BRIGHTNESS_OBSERVER_COMMAND`, an installed `brightness -l` helper, or opt-in `DYNAMAC_ENABLE_IOREG_BRIGHTNESS=1` readable backlight data. Run `npm run check-readme` to validate that this README keeps documenting the DynaKeys HUD section, local volume/brightness behavior, runnable status tests, app-mode replay-store path, and deferred native key-capture boundaries. Deferred: global keyboard shortcuts/action launchers, private Accessibility/Input-Monitoring key hooks, and specialized native vector HUD artwork are not required by this slice; the shipped path is safe local polling plus routed compact status rendering.

### DynaClip Clipboard Activity Core Model

`src/clipboard-activity.js` implements the first DynaClip clipboard activity slice as deterministic local plain-text classification and status serialization. It accepts the current macOS clipboard text observation through the local status writer, classifies recent copied content as text, link, path, code, or empty, and returns one transient `Clipboard` activity with stable `activityId`, `activityType: "clipboard"`, priority, `createdAt`, `updatedAt`, `expiresAt`, `isTransient`, serializable `status`, compact copied HUD surface, expanded preview surface, local `source`/`metadata`, empty `revealReadyPath`, and `persisted: false`.

The implemented DynaClip behavior is intentionally short-lived and in-memory by default: recent copied plain text shows a compact `Copied` HUD with the classification glyph and a sanitized preview, expanded state shows a bounded preview with character count/classification details, duplicate clipboard signatures are filtered, stale or unchanged clipboard reads remain idle, and clipboard text is not written to persistent history across restarts. The app-mode Mac activity snapshot can include this `Clipboard` status beside Now Playing and Battery without adding cloud sync, credentials, paid APIs, or invasive permissions.

Run `npm run test:clipboard-activity`, `npm run test:clipboard-activity-store`, `npm run test:clipboard-preview-ui`, `npm run test:clipboard-duplicate-filter`, and `npm run test:mac-activity-status` to verify the runnable DynaClip model, non-persistent clipboard store behavior, compact/expanded preview contract, duplicate filtering, and status integration. Run `npm run check-readme` to validate that this README keeps documenting the DynaClip section, local clipboard classification, transient copied HUD, expanded preview, non-persistence boundary, and runnable clipboard tests. Deferred: clipboard history, cross-restart clipboard persistence, Finder companion workflows, file shelf handoff, paste automation, and quick handoff actions are not implemented by this slice.

### DynaDrop/Shelf Core Model

`src/shelf-state.js` implements the DynaDrop/Shelf MVP as deterministic local shelf state, not native drag capture. `addDroppedFileToShelf` accepts a validated existing local file path plus optional MIME/type, source, and observation timestamp, then stores only reveal-safe metadata in state: `path`, `name`, `type`, `size`, source, timestamps, stable `shelf-<observedAt>-<sequence>` item IDs, `revealReadyPath`, explicit `revealStatus`, and `persisted: false`. File contents are never read into shelf state. The recoverable `applyDroppedFileToShelf` path preserves invalid dropped inputs as a stable non-persistent `lastError` value (`code`, message, input kind, observed/updated timestamps, `recoverable: true`, and unavailable `revealStatus`) without adding items or changing the active shelf activity, then clears that error after the next valid drop.

The active shelf activity serializes with stable activity fields (`activityId`, `activityType: "shelf"`, priority, timestamps, non-transient status, compact/expanded surfaces, source, metadata, `revealReadyPath`, `revealStatus`, and `persisted: false`) so the Activity Router can rank it below clipboard and above Timer. `buildShelfRevealStatus` returns deterministic `ready` status only for an existing validated local file and deterministic `unavailable` status for empty, missing, malformed, or non-file paths; unavailable states expose an empty `revealReadyPath` so stale paths are not implied. Ready reveal status is intentionally not an action contract: it carries `canExecuteReveal: false`, `canOpen: false`, and `executionState: "deferred"` because this codebase currently has media-source URL opening but no safe app-mode Finder file reveal/open pattern. `buildShelfStatusPayload` emits a schema-valid `DynaShelf` status item whose detail explicitly says native drag capture and Finder reveal/open execution are deferred. `clearShelf` removes all file metadata and the active shelf activity.
...
- DynaClip-style clipboard history, cross-restart clipboard persistence, Finder companion, file shelf, paste automation, and quick handoff workflows are out of scope beyond the tested short-lived local clipboard activity/status model.
...
- DynaKeys local volume/brightness HUD status models, system-volume polling, optional brightness observer inputs, and HUD event replay store are implemented as transient local status logic plus `.build/hud-events.json` replay for app-mode launches.
...
- [ ] Private/global key hooks and global shortcut/action launchers remain deferred; current DynaKeys ships safe system-volume polling plus optional brightness observer inputs, not Accessibility/Input-Monitoring hooks.
